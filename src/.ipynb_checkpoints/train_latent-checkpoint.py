...
def train_and_validate(generator, latent_codes, dict_latent_codes, optimizer_g, optimizer_l, scheduler_g, scheduler_l, train_loader, val_loader, start_epoch, num_epochs, checkpoints_path, latent_path, device, best_val_loss, data_path):
    ...
    timing_data = []  # Collect timing data for all epochs
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_bar:
            ...
            poch_latent_opt_time += time.time() - optimizer_l_start_time
            ...
            train_bar.set_postfix({"Batch Total Time": batch_time, "Batch Loss": loss_l.item(), "Optimizer G Time": optimizer_g_time, "Optimizer L Time": optimizer_l_time})
        ...
        print(f'Epoch {epoch+1} Total Training Time: {total_train_time:.2f} seconds, Total Validation Time: {total_validation_time:.2f} seconds')
    ...
    save_timing_data('timing_data.csv', epoch+1, total_train_time, total_validation_time, total_weight_optimization_time, total_latent_optimization_time)
