...
def save_to_hdf5(flux, wavelength, sigma, flux_mask, unique_id, instrument_type, hdf5_dir, index, latent_size, combined_meta, healpix_index, wavelength_grid):
    ...
    for i, (apogee_id, galah_id) in enumerate(tqdm(zip(apogee_ids, galah_ids), total=len(apogee_ids), desc="Converting spectra to HDF5")):
        ...
        try:
            ...
            if apogee_exists:
                ...
                flux = np.concatenate(( galah_data[0],apogee_data[0]))
                wavelength = np.concatenate(( galah_data[1],apogee_data[1]))
                sigma = np.concatenate(( galah_data[2], apogee_data[2]))
                flux_mask = np.concatenate(( galah_data[3], apogee_data[3]))
                instruments = np.concatenate(( galah_data[4], apogee_data[4]))  
            ...
            combined_meta = {
                'temp': [galah_meta['temp'], apogee_meta['temp'] if apogee_meta else None],
                'temp_err': [ galah_meta['temp_err'], apogee_meta['temp_err'] if apogee_meta else None],
                'logg': [galah_meta['logg'], apogee_meta['logg'] if apogee_meta else None],
                'logg_err': [galah_meta['logg_err'], apogee_meta['logg_err'] if apogee_meta else None],
                'rv': [ galah_meta['rv'], apogee_meta['rv'] if apogee_meta else None],
                'rv_err': [galah_meta['rv_err'], apogee_meta['rv_err'] if apogee_meta else None],
                'ra': [ galah_meta['ra'], apogee_meta['ra'] if apogee_meta else None],
                'dec': [ galah_meta['dec'], apogee_meta['dec'] if apogee_meta else None],
        ...
        save_to_hdf5(flux, wavelength, sigma, flux_mask, unique_id, instrument_type, hdf5_dir, index, latent_size, combined_meta, healpix_index, wavelength_grid)
