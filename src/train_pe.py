...
def weighted_mse_loss(input, target, weight):
    assert input.shape[:-1] == target.shape == weight.shape, f'Shapes of input {input.shape}, target {target.shape}, and weight {weight.shape} must match in all but the last dimension'
    
    # Compute MSE along the last dimension
    mse = torch.mean((input - target.unsqueeze(-1))**2, dim=-1)
...
def train_and_validate (generator, latent_codes, dict_latent_codes, optimizer_g, optimizer_l, scheduler_g, scheduler_l, train_loader, val_loader, start_epoch, num_epochs, checkpoints_path, latent_path, device, best_val_loss, data_path, plots_path, config):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_bar:
            ...
            generated = generator(latent_codes[indices],wavelength)
...
def main():
    ...
    # (latent_dim, output_dim, layers, max_wavelength=17500, activation_function='LeakyReLU', pe_dim=64)
